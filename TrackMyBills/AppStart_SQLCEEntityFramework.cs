...
[assembly: WebActivator.PreApplicationStartMethod(typeof(TrackMyBills.AppStart_SQLCEEntityFramework), "Start")]

namespace TrackMyBills
{
    public static class AppStart_SQLCEEntityFramework
    {
        public static void Start()
        {
            ...
            Database.SetInitializer(new CreateCeDatabaseIfNotExists<BillContext>());
        }
    }
    ...
    public class CreateCeDatabaseIfNotExists<TContext> : SqlCeInitializer<TContext> where TContext : DbContext
    {
        ...
        {
            var biller = new Biller { ID = Guid.NewGuid(), Name = "Mortgage" };
            var billContext = context as BillContext;
            billContext.Billers.Add(biller);
            billContext.BillOccurrences.Add(new BillOccurrence { BilledFrom = biller, DayOfMonthDue = 6, Frequency = (int)BillFrequency.Monthly, ID = Guid.NewGuid() });
            billContext.Bills.Add(new BillModel { BillerID=biller.ID, BilledFrom = biller, DueOn = new DateTime(2011, 02, 06), Amount="12.21", EnteredOn = DateTime.UtcNow, ID = Guid.NewGuid() });
